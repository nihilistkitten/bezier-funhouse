...
const SMOOTHNESS_THRESHOLD = 1 / SMOOTHNESS;
...
function recBez(p0, p1, p2, isLeft) {
  // todo: does dedup work
  if (smoothEnough(p0, p1, p2)) {
    if (isLeft) {
      return [p0, p1, p2];
    } else {
      // we arbitrarily decide the left call is responsible for adding the midpoint
      return [p1, p2];
    }
  }
  ...
  const r1 = p2.combo(0.5, p1);
  const l2 = l1.combo(0.5, r1);

  return recBez(l0, l1, l2, true).concat(recBez(l2, r1, r2, false));
}

function smoothEnough(p0, p1, p2) {
  // distance from p1 to segment p0p2
  return p2.combo(0.5, p0).minus(p1).norm() < SMOOTHNESS_THRESHOLD;
}
